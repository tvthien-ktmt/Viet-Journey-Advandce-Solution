...
export function SiteHeader() {
  ...
  const navItems = [
    { id: 'book', label: _t.header.nav.book, href: '#booking' },
    { id: 'manage', label: _t.header.nav.manage, href: '#' },
    { id: 'checkin', label: _t.header.nav.checkin, href: '#' },
    { id: 'status', label: _t.header.nav.status, href: '#' },
    { id: 'discover', label: _t.header.nav.discover, href: '#offers' },
    { id: 'destinations', label: _t.header.nav.destinations, href: '#destinations' },
  ];
  ...
  return (
    <header className="sticky top-0 z-[70] border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-white/80">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
        <Link to="/" className="flex items-center gap-3" aria-label="Vietnam Airlines — Home">
        ...
        <div className="flex items-center gap-2">
          ...
          <a href="#booking" className="flex items-center gap-2 hidden bg-[#023a78] text-white hover:bg-vna-blue-800 sm:inline-flex rounded-md transition-all duration-300 h-9 px-3 text-xs font-medium">
            ...
            <span>{_t.header.book}</span>
          </a>

          <Sheet open={open} onOpenChange={setOpen}>
              <SheetTrigger className="border-[#023a78]/30 text-[#023a78] lg:hidden p-2 rounded-md hover:bg-slate-100 transition-all duration-300" aria-label={_t.header.menu}>
              ...
              <SheetHeader className="border-b border-slate-200 bg-[#023a78] p-4 text-white">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-[#f5a623]">{_t.header.tagline}</p>
                  </div>
                </div>
              </SheetHeader>
